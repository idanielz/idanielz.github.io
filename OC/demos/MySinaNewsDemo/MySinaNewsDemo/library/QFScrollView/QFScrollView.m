...
-(void)updateLayout
{
    ...
    if (UIDeviceOrientationIsLandscape(deviceOrientation)) {
        ...
        screenBounds=CGRectMake(0, 0, screenBounds.size.width, screenBounds.size.height);
    }
    //重新设置位置和大小
    ...
    //切换到转屏前的页码
    [self changePage:curPage animated:NO];
    //加载当前可见页
    [self tilePages];
}
//删除所有子页信息
...
- (void)tilePages
{
    ...
    //跟据左下角x和右上角x计算当前可见页码
    NSInteger firstNeededPageIndex = floorf(CGRectGetMinX(visibleBounds) / self.pageSize.width);
    NSInteger lastNeededPageIndex  = floorf((CGRectGetMaxX(visibleBounds)-1) / self.pageSize.width);
    //保存当前页码
    curPage=firstNeededPageIndex;
    //预加载当前页的前后页
    firstNeededPageIndex = MAX(firstNeededPageIndex-1, 0);
    lastNeededPageIndex  = MIN(lastNeededPageIndex+1, pageCount- 1);
//    NSLog(@"%ld %ld", firstNeededPageIndex, lastNeededPageIndex);
    // 收藏不再可见页,以备复用
